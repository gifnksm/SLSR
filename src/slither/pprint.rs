...
impl Config {
    fn is_cjk(&self) -> bool { self.cjk == Type::Yes }
}
...
pub fn print(conf: &Config, board: &Board) -> IoResult<()> {
    let cjk = if conf.cjk == Type::Auto {
        let loc = locale::setlocale(Category::CType, "");
        if loc.starts_with("ja") || loc.starts_with("ko") || loc.starts_with("zh") {
            Type::Yes
        } else {
            Type::No
        }
    ...
    Table::pprint(&mut Printer::new(), &Config { cjk: cjk, .. *conf }, board)
}
